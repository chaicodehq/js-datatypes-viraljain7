...
const validCart = cart.filter(item => item && item.qty > 0);
  ...
  const items = validCart.map(item => {
    ...
    const addonTotal = addons.reduce((sum, addon) => {
      const addonPrice = Number(addon.split(":")[1]) || 0;
 ...
 // Step 4: Delivery Fee (correct order)
  let deliveryFee = 0;
...
let gst=Number((subtotal*0.05).toFixed(2))
  ...
  let discount = 0;
  const couponLower = typeof coupon === "string"
    ? coupon.toLowerCase().trim()
    : "";
...
// Step 7: Grand Total (min 0)
  const grandTotal = parseFloat(
    Math.max(subtotal + deliveryFee + gst - discount, 0).toFixed(2)
  );

console.log({
    // items[name: string, qty: number, basePrice: number, addonTotal: number, itemTotal: number ],
    items,
    subtotal,
    deliveryFee,
    gst,
